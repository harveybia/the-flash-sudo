...
class Vec2D:
    ...
    def __eq__(self,other):
        if not isinstance(other,Vec2D): return False
        return almostEqual(self.x,other.x) and almostEqual(self.y,other.y)
    ...
    def __mul__(self, other):
        if isinstance(other,Vec2D):
            result = Vec2D(1,0)
            result = result.set_magnitude(self.length() * other.length())
            result = result.set_angle(self.get_angle() + other.get_angle())
            return result
        else: return self.scale_vector(other)
    ...
    def normal(self):
        # I modified this so it doesn't give errors...
        if self.length_squared() == 0: return self
        else: return self / self.length()
    ...
    def set_magnitude(self, target_magnitude):
        ...
        else: return Vec2D(0,target_magnitude)
    ...
    # Return a reflection with the other as the reflecting surface
    # By Matt Zhao
    def reflect(self,other):
        para = self.projection_onto(other)
        perp = self - para
        return para - perp
